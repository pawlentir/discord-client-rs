...
#[cfg(test)]
mod tests {
    use super::{create_op_8, create_op_35, create_op_37};
    use crate::gateway::{GuildSubscription, GuildSubscriptions};
    use serde_json::json;
    use std::collections::HashMap;

    #[test]
    fn op_37_serializes_channel_member_ranges() {
        let channels = HashMap::from([(456, vec![[0, 99], [100, 199], [1_000, 1_099]])]);
        let subscriptions =
            GuildSubscriptions::from([(123, GuildSubscription::member_lists(channels))]);

        let payload: serde_json::Value =
            serde_json::from_str(&create_op_37(&subscriptions)).unwrap();
    ...
    #[test]
    fn op_37_preserves_legacy_bulk_subscription_payload() {
        let subscriptions = GuildSubscriptions::from([
            (123, GuildSubscription::all_events()),
            (456, GuildSubscription::all_events()),
        ]);

        let payload: serde_json::Value =
            serde_json::from_str(&create_op_37(&subscriptions)).unwrap();

        for guild_id in ["123", "456"] {
            assert_eq!(
                payload["d"]["subscriptions"][guild_id],
                json!({
                    "typing": true,
                    "threads": true,
                    "activities": true,
    ...
    #[test]
    fn op_35_serializes_continuation_and_nonce_as_discord_expects() {
        let payload: serde_json::Value =
            serde_json::from_str(&create_op_35(123, "", Some(456), Some("page-2"))).unwrap();
    ...
    #[test]
    fn op_8_serializes_prefix_limit_and_nonce_for_correlated_requests() {
        let payload: serde_json::Value = serde_json::from_str(&create_op_8(
            123,
            Some("ab"),
            Some(100),
            Some(false),
